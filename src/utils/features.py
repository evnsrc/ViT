...
def patchify(images, n_patches):
    ...
    patches = torch.zeros(n, n_patches**2, h * w * c // n_patches**2, device='cuda')
...
def depatchify(patches, n_patches, chw):
    ...
    images_recovered = torch.zeros(n, chw[0], chw[1], chw[2],  device='cuda')

    for idx, patch in enumerate(patches):
        
        image_r = torch.empty(0, chw[2],  device='cuda')
        
        for i in range(n_patches):
            #patch_r_i_1 = patch[i, :].view(patch_h, patch_w)
            row_tensor = torch.empty(patch_h, 0,  device='cuda')
            for j in range(n_patches):
