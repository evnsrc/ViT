...
new_data.to_csv('loss2.csv', index=False)

#Sauvegarder le poid 0 dans le fichier de sauvegarde
filename = f'Train0016/weight_epoch0_loss:{tr_LOSS[-1]:.11f}.pth'
torch.save(transformer.state_dict(), filename)

#save the model
torch.save(transformer.state_dict(), 'weights2.pth')
...
for epoch in range(0,100):
    ...
    filename = f'Train0016/weight_epoch{epoch + 1}_loss:{tr_LOSS[-1]:.11f}.pth'
    torch.save(transformer.state_dict(), filename)
    ...
    new_data.to_csv('loss2.csv', mode='a', header=not os.path.exists('loss2.csv'), index=False)
